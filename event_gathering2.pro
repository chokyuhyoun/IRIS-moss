...
;      print, n_elements(si_i_tot)
      ...
      pre_sg_ind = [[pre_sg_ind], [eout.pre_sg_ind]]   ; [y_pix, time in a dataset, dataset]
;      if n_elements(ar_no) gt 22 then stop 
      sg_phy = [[sg_phy], [eout.sg_phy]]
      sg_dy = [[sg_dy], replicate(eout.sg_dy, eout.moss_num)] ; to define the event!
      sg_file = [sg_file, replicate(eout.sg_files[0], eout.moss_num)]
      curve_dt = [curve_dt, replicate(eout.curve_dt, eout.moss_num)]
      print, file_basename(eout.sg_files[0])
...
;        print, eout.pre_sg_ind[1, k], eout.sg_ind[1, k], eout.aft_sg_ind[1, k]
  ...
  par_names = ['ar_no', 'sol_x', 'sol_y', 'times', 'cur_fwhm', $
    ...
    ;               'em', 'pre_em', $
    'fe18_avg']
  real = where(finite(si_i_tot))
  for ii=0, n_elements(par_names)-1 do begin
    dum = execute(par_names[ii]+'='+par_names[ii]+'[real]')
  endfor
  si_err = si_err[real]
...
;  sg_ind = sg_ind[*, real]
;  pre_sg_ind = pre_sg_ind[*, real]
;  sg_dy = sg_dy[real]
;  si_err = si_err[real]
;  mg_k_err = mg_k_err[real]
;  mg_h_err = mg_h_err[real]
;  sg_file = sg_file[real]
;  curve_dt = curve_dt[real]
  event_no = intarr(n_elements(si_err))
  no = 0
  for ii=1, n_elements(event_no)-1 do begin
;    if ii ge 1411 then stop
    near_slit_pos = where(abs(sg_phy[0, 0:ii-1] - sg_phy[0, ii]) le 1./sg_dy[ii], /null)
    successive    = where(abs(sg_ind[1, 0:ii-1] - sg_ind[1, ii]) le 1., /null)
    same_obs = where(sg_ind[2, 0:ii-1] eq sg_ind[2, ii], /null)
    intsec1 = intersect_(near_slit_pos, successive)
    intsec2 = intersect_(intsec1, same_obs)
    if n_elements(intsec2) eq 0 then begin
      no += 1
      event_no[ii] = no
    endif else begin 
      event_no[ii] = event_no[intsec2[0]]
    endelse
...
;    if ii eq 1283 then stop
    same_slit_pos = where(sg_ind[0, 0:ii-1] eq sg_ind[0, ii], /null)
    successive = where(abs(sg_ind[1, 0:ii-1] - sg_ind[1, ii]) le 1., /null)
    same_obs = where(sg_ind[2, 0:ii-1] eq sg_ind[2, ii], /null)
    intsec1 = intersect_(same_slit_pos, successive)
    intsec2 = intersect_(intsec1, same_obs)
    if n_elements(intsec2) eq 0 then begin
      no += 1
      pixel_no[ii] = no
    endif else begin 
      pixel_no[ii] = pixel_no[intsec2[0]]
    endelse
...
pars_struct = !null
for j=0, n_elements(pixel_no)-1 do begin
  com0 = !null
  for i=0, n_elements(par_names)-1 do begin
    com0 = [com0, par_names[i]+':pars['+string(j, f='(i0)')+', '+string(i, f='(i0)')+']']
  endfor
  dum = execute('pars0 = {'+strjoin(com0, ', ')+'}')
  pars_struct = [pars_struct, pars0]
endfor
...
for ii=0, max(event_no) do begin
  ...
  if count lt 2 then ind0 = event_ind[0] else begin
    max_ind = where(si_i_tot[event_ind] eq max(si_i_tot[event_ind]))
    ind0 = event_ind[max_ind[0]]
  endelse
  ref_ind = [ref_ind, ind0]
endfor
pars_ev_peak = pars[ref_ind, *]
pars_ev_peak_ind = ref_ind
...
for ii=0, max(pixel_no) do begin
  ...
  if count lt 2 then ind0 = pixel_ind[0] else begin
    max_ind = where(si_i_tot[pixel_ind] eq max(si_i_tot[pixel_ind]))
    ind0 = pixel_ind[max_ind[0]]
  endelse
;  ind0 = (count lt 2 ) ? pixel_ind[0] : (where(si_i_tot eq max(si_i_tot[pixel_ind])))[0]
;  if ind0 lt min(pixel_ind) or ind0 gt max(pixel_ind) then stop
  ref_ind = [ref_ind, ind0]

  
  if count lt 2 then ind1 = pixel_ind[0] else begin
    max_ind = where(mg_k_tot[pixel_ind] eq max(mg_k_tot[pixel_ind]))
    ind1 = pixel_ind[max_ind[0]]
  endelse
;  ind1 = (count lt 2 ) ? pixel_ind[0] : where(mg_k_tot eq max(mg_k_tot[pixel_ind]))
  if ind1 lt min(pixel_ind) or ind1 gt max(pixel_ind) then stop
  ref_ind_mg = [ref_ind_mg, ind1]

endfor
pars_pix_peak = pars[ref_ind, *]
;pars_pix_peak = pars_struct[ref_ind, *]
pars_pix_peak_ind = ref_ind

if 0 then begin ;; mg analysis using Mg k peak moment

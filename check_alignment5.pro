...
sav_file = file_search(dir, 'events_hcr_*.sav')
restore, sav_file
...
sg_files = eout.sg_files
read_iris_l2, aia_file, aia_index, aia_data, /sil
read_iris_l2, sji_file, sji_index, sji_data, /sil
...
for i=0, n_elements(sg_index)-1 do begin
;  i = 0
  ...
  del = [[del], [alignoffset(aia_img, sji_corr_aia)]]
endfor
;stop
...
;;; ----------
;
;;xshift = eout.xshift_iris2aia
;;yshift = eout.yshift_iris2aia
;
;
;
;w3 = window(dim=[12d2, 4d2])
;p031 = plot(sji_time-sji_time[0], xshift, /current, $
;            position=[0.1, 0.15, 0.3, 0.9], aspect_ratio=0, $ 
;            xtitle='Time (s)', ytitle='X shift (arcsec)')
;p032 = plot(sji_time-sji_time[0], yshift, /current, $
;            position=[0.43, 0.15, 0.63, 0.9], aspect_ratio=0, $
;            xtitle='Time (s)', ytitle='y shift (arcsec)')
;
;            
;p033 = plot(xshift, yshift, '-+', pos=[0.73, 0.15, 0.96, 0.9], /current, $
;           rgb_table=39, vert_colors=bytscl(findgen(n_elements(xshift))),$
;           xtitle = 'X shift (arcsec)', $ 
;           ytitle = 'Y shift (arcsec)', $
;           title = 'Pointing Difference between SJI 1400 $\AA$ and AIA 1600 $\AA$', $
;           font_size=15, aspect_ratio=1)

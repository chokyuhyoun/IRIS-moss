PRO get_iris_from_mossevent,hcr,outdir,output,mash_out,checkplot=checkplot,savesample=savesample,movie=movie,lightcurve=lightcurve
...
for i=0,nresults-1 do begin
    ...
    if mash['status'] eq 'data ok' then begin
        ...
        ;get SJI index - any one will do? just need the x slit position
        ...
        ;time dependent IRIS slit position
        sji_slit = index_sji.xcen
        sji_slit_xpos = 0

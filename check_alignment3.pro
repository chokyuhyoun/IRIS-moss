...
sav_file = file_search(dir, '*event3.sav')
restore, sav_file
...
;for i=0, n_elements(eout.sg_time)-1 do begin
  ...
  aia_xpdata = (findgen(aia_sz[1])-(aia_sz[1]-1)*0.5)*aia_index[match_aia].cdelt1 $
                + aia_index[match_aia].crval1 - eout.xshift_iris2aia
  aia_ypdata = (findgen(aia_sz[2])-(aia_sz[2]-1)*0.5)*aia_index[match_aia].cdelt2 $
                + aia_index[match_aia].crval2 - eout.yshift_iris2aia
...
video = idlffvideowrite('align3.mp4')
wdims = w1.dimensions
...
im33.save, 'align3.png', resol=200
end
...
save_dir = dir+'using_mean/'
file_mkdir, save_dir
how_to_select = 'mean value of event'
overall = 1 
all_scatter = 1
all_density = 1
...
if all_scatter then begin
  ...
  for j=0, n_params-1 do begin ; y direction
    for i=0, n_params-1 do begin ; x direction
      ...
      for l=0, n_elements(ars)-1 do begin
        xselected = pars_event_mean.(xpar)[where(pars_event_mean.ar_no eq ars[l])]
        yselected = pars_event_mean.(ypar)[where(pars_event_mean.ar_no eq ars[l])]
...
if all_density then begin
  ...
  for j=0, n_params-1 do begin ; y direction
    for i=0, n_params-1 do begin ; x direction
      xpar = params[i]
      ypar = params[j]
      xparam = pars_event_mean.(xpar)
      yparam = pars_event_mean.(ypar)
